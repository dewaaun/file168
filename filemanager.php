...
function fix_filename_download($str){ // no download podemos ter acentos
    $str = trim($str);// Substitui caracteres reservados
    $str = str_replace('\\', ' ', $str);
    $str = str_replace('/', ' ', $str);
    $str = str_replace(':', ' ', $str);
    $str = str_replace('*', ' ', $str);
        ...
        if($entry_count){
            ...
            foreach ($entry_list as $ind=>$dir_entry) {
                ...
                if ($dir_entry["type"] == "dir") {
                    ...
                    $dir_out[$dir_count][] = "<td class=\"sm\"><nobr>".$dir_entry["p"]."</td>";if (!$is_windows) {
...
class archive {
    ...
    function list_files($list) {
        ...
        foreach ($list as $current) {
            ...
            $current = preg_replace("/\/$/", "", $current);
            if (strstr($current, "*")) {
                $regex = preg_replace("/([\\\^\$\.\[\]\|\(\)\?\+\{\}\/])/", "\\\\\\1", $current);$regex = str_replace("*", ".*", $regex);
...
function et($tag){
    ...
    $et['pl']['DestEqOrig'] = 'Folder docelowy jest równybieżącemu folderowi';
